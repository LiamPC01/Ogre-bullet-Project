...
Game::Game() : ApplicationContext("MoogEngine")
{
	dynamicsWorld = NULL;
...
void Game::setupCamera()
{
	...
	camNode->setPosition(100, 1000, 800);
	camNode->lookAt(Vector3(0, 250, 0), Node::TransformSpace::TS_WORLD);
...
void Game::setupBoxMesh()
{
	...
	thisSceneNode->setPosition(500, 100, -500);
...
void Game::setupBoxMesh2()
{
   ...
   // I'll rotate the scene node and later the collision shape.
	...
	thisSceneNode->setPosition(200, 1600, 0);
...
bool Game::frameStarted(const Ogre::FrameEvent& evt)
{
	...
	if (this->dynamicsWorld != NULL)
	{
		...
		if (player->ninjaAnimationState->getEnabled() == false)
		{
...
bool Game::frameEnded(const Ogre::FrameEvent& evt)
{
	if (this->dynamicsWorld != NULL)
	{
		...
		if (player->ninjaAnimationState->hasEnded() == true) {
			player->ninjaAnimationState->setEnabled(false);
		}
...
{
	// Setup Abient light
	scnMgr->setAmbientLight(ColourValue(7.0f, 7.0f, 7.0f));
