...
void Player::forward()
{
	...
	//pointing down z.
	btVector3 fwd(0.0f, 0.0f, 0.0f); // -forwardForce
	btVector3 push;
...
{
	btVector3 back(0.0f, 0.0f, forwardForce);
	btVector3 push;
	...
	if (body && body->getMotionState())
	{
		...
		//rotate the local force, into the global space.
		//i.e. push in down the local z.
		push = quatRotate(orientation, back);
...
void Player::setUpNinjaAnimation() {
	ninjaAnimationState = box->getAnimationState("Idle1");
	ninjaAnimationState->setLoop(true);
	ninjaAnimationState->setEnabled(true);
}

void Player::playAnimation(const FrameEvent& evt) {
	ninjaAnimationState->addTime(evt.timeSinceLastFrame);
}

void Player::walkAnimation() {
	ninjaAnimationState = box->getAnimationState("Walk");
	ninjaAnimationState->setLoop(true);
	ninjaAnimationState->setEnabled(true);
...
void Player::idleAnimation() {
	ninjaAnimationState = box->getAnimationState("Idle2");
	ninjaAnimationState->setLoop(true);
	ninjaAnimationState->setEnabled(true);
}

void Player::attackAnimation() {
	ninjaAnimationState = box->getAnimationState("Attack1");
	ninjaAnimationState->setTimePosition(0);
	ninjaAnimationState->setLoop(false);
	ninjaAnimationState->setEnabled(true);
